        ...
        */
        void forward(double left_speed, double right_speed)
        {
            if (this->m_isConnectionOk >= 0) 
            {
                ...
                double lspeedfreq = std::max(std::min(((left_speed / m_maxSpeed) * m_maxfreq), static_cast<double>(m_maxfreq)), static_cast<double>(0));
                double rspeedfreq = std::max(std::min(((right_speed / m_maxSpeed) * m_maxfreq), static_cast<double>(m_maxfreq)), static_cast<double>(0));
                ...
                set_PWM_dutycycle(m_isConnectionOk,this->m_right_step, rspeedfreq);
